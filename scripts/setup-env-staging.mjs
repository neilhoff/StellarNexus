/**
 * Interactive setup for Lambda environment variables in the staging environment.
 *
 * Uses `arc env --add --env staging` to store vars in AWS SSM Parameter Store.
...
async function main () {
  ...
  try {
    ...
    const confirm = (await rl.question(`\nWrite to AWS SSM (${ENVIRONMENT})? (y/N): `)).trim().toLowerCase()
    ...
    if (successCount > 0) {
      ...
      console.log('Deploy to apply: npx arc deploy')
    }
  } catch (error) {
